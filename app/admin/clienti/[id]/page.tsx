    ...
    <div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="space-y-5">
          ...
          <div className="card p-5">
            <h2 className="font-semibold text-gray-900 mb-4">Anagrafica</h2>{/* TODO: i18n */}
          ...
          {/* Abbonamento */}
          <div className="card p-5">
            <h2 className="font-semibold text-gray-900 mb-4">Abbonamento</h2>{/* TODO: i18n */}
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-500">Piano</span>{/* TODO: i18n */}
              ...
              {host.dataInizioAbb && (
                <div className="flex justify-between">
                  <span className="text-gray-500">Inizio</span>{/* TODO: i18n */}
                  <span>{formatData(host.dataInizioAbb)}</span>
                </div>
              )}
              {host.dataFineAbb && (
                <div className="flex justify-between">
                  <span className="text-gray-500">Scadenza</span>{/* TODO: i18n */}
                  <span>{formatData(host.dataFineAbb)}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-gray-500">Cliente dal</span>{/* TODO: i18n */}
          ...
          {host.note && (
            <div className="card p-5">
              <h2 className="font-semibold text-gray-900 mb-2">{tc('notes')}</h2>
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <div className="card overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              <h3 className="font-semibold text-gray-900">{ti('title')} ({host.fatture.length})</h3>
              <Link href={`/admin/fatture?hostId=${host.id}`} className="text-sm text-brand-600 hover:text-brand-700 font-medium">Vedi tutte →</Link>{/* TODO: i18n */}
            </div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[640px]">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    <th className="table-th">{ti('number')}</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {host.fatture.length === 0 ? (
                    ...
                    <tr key={f.id} className="hover:bg-gray-50">
                      ...
                      <td className="table-td"><Link href={`/admin/fatture/${f.id}`} className="text-brand-600 text-sm hover:underline">{tc('open')}</Link></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="card overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              <h3 className="font-semibold text-gray-900">{tp('title')} ({host.pagamenti.length})</h3>
              <Link href={`/admin/pagamenti?hostId=${host.id}`} className="text-sm text-brand-600 hover:text-brand-700 font-medium">Vedi tutti →</Link>{/* TODO: i18n */}
            </div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[640px]">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    <th className="table-th">{tp('description')}</th>
                    <th className="table-th">{tc('amount')}</th>
                    <th className="table-th">{tp('dueDate')}</th>
                    <th className="table-th">{tc('status')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
      ...
      <div className="card p-6 mt-6">
        <div className="flex items-center justify-between mb-1">
          <h2 className="font-semibold text-gray-900">Canali di comunicazione</h2>{/* TODO: i18n */}
          <span className="text-xs bg-amber-100 text-amber-700 font-medium px-2.5 py-0.5 rounded-full">
            Solo admin{/* TODO: i18n */}
          </span>
        </div>
