...
export default async function PacchettiPubbliciPage({
  ...
  const params = await paramsPromise
  const t = await getTranslations('booking.packages')
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-4xl mx-auto px-4 py-6">
          ...
          >
            <ArrowLeft className="w-4 h-4" /> {t('backToStructure')}
          </Link>
          <h1 className="text-2xl font-bold text-gray-900">
            {t('title')} — {struttura.nome}
          </h1>
                ...
                <div className="px-6 py-5 space-y-4">
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-500">
                    <span className="flex items-center gap-1">
                      ...
                      {p.notti} {p.notti === 1 ? tc('night') : tc('nights')}
                    </span>
                    <span className="flex items-center gap-1">
                      <Users className="w-4 h-4 text-indigo-500" />
                      {p.numOspiti} {tc('guests')}
                    </span>
                  </div>
                  ...
                  {p.incluso.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">{t('includes')}</p>
                  ...
                  {(p.dataInizio || p.dataFine) && (
                    <p className="text-xs text-gray-400 flex items-center gap-1">
                      ...
                      {t('valid')} {p.dataInizio ? `${t('validFrom')} ${formatData(p.dataInizio)}` : ''}
                      {p.dataFine ? ` ${t('validTo')} ${formatData(p.dataFine)}` : ''}
                    </p>
                  )}
                  ...
                  <div className="flex items-end justify-between pt-3 border-t border-gray-100">
                    <div>
                      ...
                      <span className="text-xs text-gray-400 ml-1">{t('perPackage')}</span>
                    </div>
                    <Link
