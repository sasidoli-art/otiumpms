...
export default async function HostAnalyticsPage() {
  ...
  const t = await getTranslations('host.analytics')
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-8">
        <StatCard titolo={t('activeEvents')} valore={eventi.filter(e => e.stato === 'APPROVATO').length} icona={<CalendarDays size={20} />} colorIcona="bg-brand-100 text-brand-700" />
        <StatCard titolo={t('totalViews')} valore={totaleViews.toLocaleString('it-IT')} icona={<Eye size={20} />} colorIcona="bg-blue-100 text-blue-700" />
        <StatCard titolo={t('totalClicks')} valore={totaleClick.toLocaleString('it-IT')} icona={<MousePointerClick size={20} />} colorIcona="bg-green-100 text-green-700" />
        <StatCard titolo={t('avgCtr')} valore={`${ctr}%`} sotto={t('clickThrough')} icona={<TrendingUp size={20} />} colorIcona="bg-purple-100 text-purple-700" />
      </div>

      {/* Grafico ultimi 30 giorni */}
      {chartData.length > 0 && (
        <div className="card p-6 mb-6">
          <h2 className="font-semibold text-gray-900 mb-4">{t('last30days')}</h2>
      ...
      <div className="card overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="font-semibold text-gray-900">{t('performanceByEvent')}</h2>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="table-th">{t('ctr')}</th>
